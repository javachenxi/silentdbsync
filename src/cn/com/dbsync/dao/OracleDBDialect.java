 ...
 * </p>
 * <p>
 * Description:关于Oracle数据库本地化处理(支持ojdbc6)
 * </p>
 * <p>
 ...
 * @created 2016 /5/31
 */
public class OracleDBDialect extends DBDialect {
     ...
     */
    public OracleDBDialect(String appCharset, String dbCharset) {
        ...
        switch (sqlType) {
            ...
            case Types.BLOB:
                ...
                if(paramVal instanceof byte[]){
                    byte[] tmpbytes = (byte[])vobj;
                    //prepareStatement.setBytes(pIndex, tmpbytes);
                    if(tmpbytes.length < BLOB_STR_MAX ){
                        prepareStatement.setBytes(pIndex, tmpbytes);
                    }else {
                        //OJDBC6的处理方式
                        blob = prepareStatement.getConnection().createBlob();
                        blob.setBytes(1, tmpbytes);
                    ...
                    try {
                        ...
                        //按BLOB进行分包保存
                        byte[] tmpbytes = CommUtil.readInputStream(inputStream, (int)tmpFile.length());
            ...
            case Types.CLOB:
                ...
                if(paramVal instanceof String){
                    ...
                    if(clobstr.length() < CLOB_STR_MAX){
                        ...
                        clob = prepareStatement.getConnection().createClob();
                    ...
                    try {
                        ...
                        String tmpStr = CommUtil.readCharacterStream(fileReader, (int)tmpFile.length());
                        if(tmpStr.length() < CLOB_STR_MAX){
                            prepareStatement.setString(pIndex, tmpStr);
                        }else {
                            clob = prepareStatement.getConnection().createClob();
    ...
    public DBSyncConstant.DBType getDBType(){
        return DBSyncConstant.DBType.ORACLEDB;
    }
